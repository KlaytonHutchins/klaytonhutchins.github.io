---
layout: default
title: Object Initialization
---

## Object Initialization

In this chapter, we will learn more about objects and classes, including reasons behind certain design decisions.

### Objectives

* Understand the use of `this.` and `this()`
* Be able to initialize fields with instance initializers.
* Understand class and object order of initialization.
* Understand the use of `final` with fields and method parameters.

### Topics

*  [`this` vs. `this()`](this-vs-this.md) 
*  [Initializing Static Fields](init-static.md) 
*  [Initializing Instance Fields](init-instance.md) 
*  [Order of Initialization](order-of-init.md) 
*  [Constant Fields and Variables](final.md) 
*  [`private` Methods](private-methods.md) 
*  [Labs](labs.md) 

<hr>

[Up](../README.md) -- [Next](this-vs-this.md)
