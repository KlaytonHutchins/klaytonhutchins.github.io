## Collection Sorting and Tuning

### Objectives

* Use the `Comparable` and `Comparator` interfaces to sort your collections.

* Use the various methods available in the `Collections` and `Arrays` utility classes.

### Topics

*  [Sorting with Comparable](SortingwithComparable.md) 

*  [Sorting with Comparator](SortingwithComparator.md) 

*  [Sorting Lists](SortingLists.md) 

*  [Sorting Maps](SortingMaps.md) 

*  [Sorting Arrays](ArraysClass.md) 

*  [`Collections` Utility Methods](CollectionsClass.md) 

*  [Labs](labs.md) 



<hr>

[Up](../README.md) -- [Next](SortingwithComparable.md)
