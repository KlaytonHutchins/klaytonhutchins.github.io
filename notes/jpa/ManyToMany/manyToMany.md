...
### Drill
> 1. In *Actor* add a field of type `List<Film>` named `films`. Annotate it with
`@ManyToMany`. Let’s choose *Actor* as the owning side of this
relationship, so it will define the `@JoinTable` annotation as well.
>
> 1. Since *Film* is the non-owning side, add a `mappedBy` element within its `@ManyToMany`. As in the example, the value of the `mappedBy` should match the name of the corresponding field in *Actor*.
>
> 1. Generate get and set methods for your new fields. Then, update your JUnit
