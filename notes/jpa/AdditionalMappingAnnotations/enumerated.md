...
Most of the time it would be better to store the actual *enum* name, instead.
  ...
  * Annotate your entity's field (not the *enum* itself) with `@Enumerated(EnumType.STRING)` to override the default ordinal behavior.
...
>    * G, PG, PG13, R, NC17  
>    ![Rating Enum](images/ratingEnum.png)
> 1. Next, modify your *Film* entity and add a new field for the film’s rating.
>    * Don’t forget to annotate the field with `@Enumerated` and provide the `EnumType.String`
...
>    ```
>
> 1. Modify your `FilmTest` class, write a test called `films_have_ratings` which ensures that a given film has the expected rating.
>
>    * Don't forget, the returned value you are testing for will be an *enum*, not a *String*.
